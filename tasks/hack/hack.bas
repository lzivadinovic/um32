...
CDLXXXVI  REM ###### NEW CODE BELOW! ###############
CDLXXXVII REM ######################################
D         REM i goes up to LIII
DX        REM      XLVIII // 48 - 0
DXX       REM      LVII // 57 - 0
DXXX      i = I
...
DLX       j = XLVIII
DLXX      k = XLVIII
DLXXX     IF CHECKPASS(username, words(i)+CHR(j)+CHR(k)) THEN GOTO M
DXC       k = k + I
DC        IF k > LVII THEN GOTO DCXX
DCX       GOTO DLXXX
DCXX      k = XLVIII
DCXXX     j = j + I
DCXL      IF j > LVII THEN GOTO DCLX
DCL       GOTO DLXXX
DCLX      i = i + I
DCLXI     k = XLVIII
DCLXII    j = XLVIII
DCLXX     IF i > pwdcount THEN GOTO MCX
DCLXXX    GOTO DLXXX
M         PRINT "found match!! for user " + username + CHR(X)
MX        PRINT "password: " + words(i)+CHR(j)+CHR(k) + CHR(X)
MXX       END
MCX       PRINT "NO password found for user " + username + CHR(X)
MCXX      END